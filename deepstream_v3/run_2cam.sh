#!/bin/bash
# 2 videoni (9-G + 11-G) parallel davomat qilish — DeepStream 8.0 (ds3).
# Ishlatish:  bash deepstream_v3/run_2cam.sh
set -e
cd "$(dirname "$0")/.."   # loyiha ildiziga

echo "[1/4] Dars jadvali sanasini bugunga tenglash (aktiv dars uchun)..."
...
echo "[2/4] Eski test davomatini tozalash (9-G cam1, 11-G cam2)..."
docker exec school_ai_web python3.14 manage.py shell -c "
from django.utils import timezone
...
echo "[3/4] Pipeline ishga tushmoqda (2 manba: sinf.mp4 + 11g.mp4)..."
...
docker run -d --name school_ai_ds3_run --gpus all \
  ...
  school_ai_ds3:latest \
  --video /data/sinf.mp4 /data/11g.mp4 >/dev/null

echo "[4/4] Tayyor! ~30 soniyada bolalar tanila boshlaydi."
echo ""
echo "  KO'RISH:"
echo "   Jonli video (9-G):  http://localhost:8554/mjpeg"
echo "   Davomat 9-G:        http://127.0.0.1:8000/monitoring/room/1/"
echo "   Davomat 11-G:       http://127.0.0.1:8000/monitoring/room/2/"
echo ""
echo "  Loglar:     docker logs -f school_ai_ds3_run"
